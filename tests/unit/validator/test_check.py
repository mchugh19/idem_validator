...
def test_output_content(hub):
    ...
    runs = {
        ...
        'fail 1': {
            'status': 'Fail: some reason',
            'duration': 0.0001
        }
    }
    hub.validator.RUNS = runs.copy()
    runs.update(
        {"TEST RESULTS": {"Passed": 2, "Failed": 1, "Skipped": 1,}}
    )
...
def test_output_empty(hub):
    ...
    '''
    check._generate_result_summary(hub)
    assert hub.validator.RUNS == {'TEST RESULTS': {'Failed': 0, 'Passed': 0, 'Skipped': 0}}
